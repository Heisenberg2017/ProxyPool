...
class RedisClient(object):
    ...
    def add(self, proxy, score=INITIAL_SCORE):
        ...
        if not re.match('\d+\.\d+\.\d+\.\d+\:\d+', proxy):
            logger.debug(f'代理不符合规范 {proxy} 丢弃')
            return
        if not self.db.zscore(REDIS_KEY, proxy):
            logger.info(f'ZADD {REDIS_KEY} {score} {proxy}')
    ...
    def decrease(self, proxy):
        ...
        if score and score > MIN_SCORE:
            logger.debug(f'代理 {proxy} 当前分数 {score} 减1')
            return self.db.zincrby(REDIS_KEY, proxy, -1)
        else:
            logger.debug(f'代理 {proxy} 当前分数 {score} 移除')
    ...
    def max(self, proxy):
        ...
        :return: 设置结果
        """
        logger.debug(f'代理 {proxy}, 可用，设置为 {MAX_SCORE}')
